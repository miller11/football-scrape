...
for year in range(int(os.getenv('START_YEAR', 1992)), int(os.getenv('END_YEAR', 2020))):
    params = {"filters.seasontype": "1", "filters.scope": "2", "filters.subscope": "1", "filters.startweek": "1",
              "filters.endweek": "17", "filters.aggregatescope": "1", "filters.range": "1", 'filters.season': year}
    ...
    print('Player Fantasy Week stats processed ({}) for: {}. CPU%: {}. Memory: {}'.format(data_json['Total'], year, psutil.cpu_percent(), dict(psutil.virtual_memory()._asdict())))
